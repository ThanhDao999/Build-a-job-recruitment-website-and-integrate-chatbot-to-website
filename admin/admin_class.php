...
Class Action {
	...
	function save_vacancy(){
	    ...
	    if($_FILES['logo_company']['tmp_name'] != ''){
			$fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['logo_company']['name'];
			$move = move_uploaded_file($_FILES['logo_company']['tmp_name'],'assets/img/'. $fname);
	    $data .= ", logo_company = '$fname' ";
		}
		$data .= ", time_company = '$time_company' ";
		$data .= ", date_company = '$date_company' ";
		$data .= ", address_company = '$address_company' ";
		$data .= ", email_company = '$email_company' ";
		$data .= ", money = '$money' ";
